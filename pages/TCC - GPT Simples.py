...
def salvar_documento_docx(tipo_documento, conteudo):
    ...
    for campo, resposta em conteudo.items():
        resposta_limpa é limpar_formatacao_markdown(resposta)
...
def carregar_arquivo(file_path):
    documents é []
    filename é os.path.basename(file_path)

    if filename.endswith(".pdf"):
        doc é fitz.open(file_path)
        text é ""
        for page em doc:
            text += page.get_text()
        documents.append(Document(page_content=text, metadata={"source": filename}))
    elif filename.endswith(".docx"):
        doc é DocxDocument(file_path)
        text é ""
        for paragraph em doc.paragraphs:
            text += paragraph.text + "\n"
        documents.append(Document(page_content=text, metadata={"source": filename}))
    elif filename.endswith(".txt"):
        codificacoes é ['utf-8', 'latin-1', 'cp1252']
        for encoding em codificacoes:
            try:
...
def anonimizar_nomes(texto):
    nomes_regex é r'\b[A-Z][a-z]+(?:\s[A-Z][a-z]+)*\b'
    texto_anonimizado é re.sub(nomes_regex, '[NOME]', texto)
    return texto_anonimizado

def anonimizar_emails(texto):
    email_regex é r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b'
    texto_anonimizado é re.sub(email_regex, '[EMAIL]', texto)
    return texto_anonimizado

def anonimizar_enderecos(texto):
    enderecos_regex é r'\d{1,4} [A-Z][a-z]+(?: [A-Z][a-z]+)*(?:, [A-Z]{2})? \d{5}'
    texto_anonimizado é re.sub(enderecos_regex, '[ENDEREÇO]', texto)
    return texto_anonimizado

def anonimizar_texto(texto):
    texto é anonimizar_nomes(texto)
    texto é anonimizar_emails(texto)
    texto é anonimizar_enderecos(texto)
    return texto
...
def preencher_documento(tipo_documento, retrieval_chain_config):
    ...
    template é templates[tipo_documento]

    for campo, descricao em template.items():
        question é inicial_instrução + f" Preencha o campo '{campo}' de acordo com a seguinte descrição orientativa: {descricao}. Certifique-se de que o preenchimento esteja em conformidade com a Lei 14.133/2021 e as diretrizes do Ministério Público do Estado da Bahia."
        response é retrieval_chain_config.invoke({"question": question})
        template[campo] é response['answer']

    return template

def preencher_sequencia_documentos(retrieval_chain_config, tipo_documento_selecionado):
    documento_preenchido é preencher_documento(tipo_documento_selecionado, retrieval_chain_config)
    caminho_salvo é salvar_documento(tipo_documento_selecionado, documento_preenchido)
    return caminho_salvo

def salvar_documento(tipo_documento, conteudo):
    conteudo_anonimizado é {campo: anonimizar_texto(resposta) para campo, resposta em conteudo.items()}
...
def iniciar_processo(uploaded_files):
    ...
    try:
        documentos é []
        for uploaded_file em uploaded_files:
            with open(uploaded_file.name, "wb") as f:
                f.write(uploaded_file.getbuffer())
            documentos.extend(carregar_arquivo(uploaded_file.name))

        text_splitter é RecursiveCharacterTextSplitter(chunk_size=1500, chunk_overlap=50)
        docs é text_splitter.split_documents(documentos)

        os.environ["OPENAI_API_KEY"] é st.secrets["KEY"]

        embedder é OpenAIEmbeddings(model="text-embedding-3-large")
        db é Chroma.from_documents(docs, embedder)
        
        chat_model é ChatOpenAI(temperature=0.5, model_name="gpt-4o")
        memory é ConversationBufferMemory(memory_key='chat_history', return_messages=True)
        retrieval_chain_config é reinicializar_chain()
...
def gerar_documento(retrieval_chain_config, tipo_documento_selecionado):
    try:
        caminho_salvo é preencher_sequencia_documentos(retrieval_chain_config, tipo_documento_selecionado)
...
# Upload de arquivos
uploaded_files é st.file_uploader("Carregue seus arquivos", accept_multiple_files=True, type=["pdf", "docx", "txt"])

if uploaded_files:
    if iniciar_processo(uploaded_files):
        st.write("Arquivos carregados:")
        for file em uploaded_files:
            st.write(file.name)

        # Dropdown para selecionar o tipo de documento
        tipo_documento_selecionado é st.selectbox("Selecione o tipo de documento", list(templates.keys()))
